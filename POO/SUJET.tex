...
%                   Barème                     %
%----------------------------------------------% 
\baremeDefautS{b=4,m=-2,e=0,v=0} % barème questions simples
...
\exemplaire{55}{ % nombre de copies différentes
...
\item En haut à droite de vos feuilles : verifiez que le premier des trois numéros soit toujours le même.
...
On considère les notions de classe et d'objet. Quelle est la proposition correcte ?
	\begin{reponses}
		\bonne{Une classe est la description d'un ensemble d'objets qui partagent les mêmes attributs et méthodes}
...
Considérons une application de gestion de location de vélos. On souhaite pouvoir modéliser plusieurs types de vélos (vélo de de ville, VTT, vélo enfant) qui possèdent à la fois des propriétés communes et des propriétés particulières à chaque type de vélo. Quelle est la meilleure proposition ?
    \begin{reponses}
...
En C++, si j’écris \textbf{class LivreNumerique: public Livre :}
    \begin{reponses}
        \bonne{\textbf{Livre} dérive de \textbf{LivreNumerique}}
        \mauvaise{\textbf{LivreNumerique} est une classe fille de \textbf{Livre}}
        \mauvaise{\textbf{LivreNumerique} est composé d’un \textbf{Livre}}
        \mauvaise{\textbf{LivreNumerique} hérite de \textbf{Livre}}
    \end{reponses}
\end{questionmult}
...
On considère le diagramme de classe ci-après figure \ref{fig:diag_classe}. Donner la description des classes à implémenter
dans un fichier .h. (Il vous est conseillé d’écrire le fichier .h par vous-même avant de choisir la bonne réponse
et de choisir ensuite une des propositions présentées ci-après figure \ref{fig:codes_h}.
    \begin{reponses}
        \mauvaise{code3.h}
        \bonne{code1.h}
        \mauvaise{code2.h}
...
\begin{figure}[h!]
	...
	\caption{Propositions de codes .h}
	\label{fig:codes_h}
\end{figure}
...
\AMCOpen{lines=1, dots=false, lineheight=18cm}{\mauvaise[F]{F}\bareme{0}\mauvaise[P]{P}\bareme{1}\mauvaise[B]{B}\bareme{3}\bonne[TB]{TB}\bareme{4}}
